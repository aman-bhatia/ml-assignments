...
gaus_kernel = lambda x,y : np.exp( - np.linalg.norm(x-y)**2 / (2*(gamma**2)))
...
# for i in range(m):
# 	for j in range(m):
# 		K[i,j] = lin_kernel(X[i], X[j])

# pkl_handle = open('K_lin.pkl','wb')
# pickle.dump(K,pkl_handle)
# pkl_handle.close()

pkl_handle = open('K_lin.pkl','rb')
...
sol_handle = open('sol_lin.pkl','wb')
pickle.dump(solution,sol_handle)
sol_handle.close()
'''
sol_handle = open('sol_lin.pkl','rb')
...
sv_file = open('support_vector_cvx_linear.txt','wb')
temp = ''
for i in range(m):
	if (sv_temp[i]):
		temp += (str(X[i])+'\n')
...
# Weight vector
W = sum(alphas[i] * Y[i] * X[i] for i in range(m))
print('Weight Vector : ', W)

# intercept
b1 = [np.dot(W,X[i]) if (Y[i]==1) else sys.maxint for i in range(m)]
b2 = [np.dot(W,X[i]) if (Y[i]==-1) else -sys.maxint for i in range(m)]
b = (-0.5) * (min(b1) + max(b2))
print('Intercept : ', b)
