...
trn = pd.read_csv('../data/data_EM/train-m2.data',delimiter=' ')
...
for t in [H,B,L,F,X]:
	t.printTable()
# print('Initial Log likelihood = ',logLikelihood())
...
while not converge:
	...
	for i in range(num_ex):
		...
		if (len(index) == 1):
			...
			data[i][index[0]] = 2
			data[i][index[1]] = 2
			new_data[-4][-1] = w0/(w0+w1+w2+w3)
			new_data[-3][-1] = w1/(w0+w1+w2+w3)
			new_data[-2][-1] = w2/(w0+w1+w2+w3)
			new_data[-1][-1] = w3/(w0+w1+w2+w3)
